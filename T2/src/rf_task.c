...
void rf_update_module_alive_timer();
...
static  const LINE_T lines_to_recv[] = {
	...
	//	{  TABLET_MSG_START_CHAR, TABLET_MSG_END_CHAR,     tablet_msg_recv_callback },
		{  LORA_MSG_START_CHAR,    LORA_MSG_END_CHAR,      lora_msg_recv_callback   },
		{       '#'     ,                   '!'    ,       set_msg_recv_callback    }
};
/*******************************/
...
TIMER_INFO_T rf_module_alive_timer;
/************************************************** */
...
//	Set_Timer(&trailer_detection_delay_timer, TRILER_DETECTION_DELAY_TIME);
	...
	Set_Timer(&rf_module_alive_timer, LE50_ALIVE_CHECK_TIMEOUT);
...
bool task_rf()
{
	...
	if(le50_is_mounted()){
		...
		/* if(mn_timer_expired(&sensor_over_mobile_app_timer)){
			 ...
			 Set_Timer(&sensor_over_mobile_app_timer, 100);*/
		 if(mn_timer_expired(&rf_module_alive_timer)){
			 le50_toggle_power();
			 Set_Timer(&rf_module_alive_timer, LE50_ALIVE_CHECK_TIMEOUT);
		 }
   /*  }*/
...
/************************************************/
void rf_update_tablet_app(RF_NODE_T *node)
{
...
/*	if(rf_get_node_presence_status()){
		tablet_event.event_type =MOBILE_APP_EVENT_XMIT_SENSOR_MESSAGE;
		json_populate_sensor_info(node, &tablet_event.json_data_t.json_type_sensor);
...
//	char buffer[256];
	...
	/*strcat(buffer,"\"Id\":\"");
	strcat(buffer, node->node_id);
	strcat(buffer,"\"");*/

	strcpy(json_sensor->id, node->node_id);
	uint8_t table_size = sizeof(sensor_type_def_t)/sizeof(sensor_type_def_t[0]);
	for(i =0; i < table_size; i++){
		json_fill_sensor_items(json_sensor, node, i);
	}
...
{
/*	strcat(buffer,"{");
	json_add_sensor_type(json_sensor->sensor.type,  sensor_type_def_t[indx]);
	strcat(buffer,",");
	json_add_sensor_index(buffer, sensor_type_def_t[indx]);
	strcat(buffer,",");
	json_add_sensor_unit(buffer,  sensor_type_def_t[indx]);
	strcat(buffer,",");
	json_add_sensor_value(buffer, sensor_type_def_t[indx]);
	strcat(buffer, "}");*/

		json_sensor->sensor[indx].index = sensor_type_def_t[indx].index;
		strcpy(json_sensor->sensor[indx].type,  sensor_type_def_t[indx].type);
		strcpy(json_sensor->sensor[indx].unit,  sensor_type_def_t[indx].unit);
		json_sensor->sensor[indx].value = sensor_type_def_t[indx].fill_value_cb(node, sensor_type_def_t[indx].index - 1);
...
{
	return node->ds18b20_channel[indx];
}
/******************************************************/
uint16_t door_value_cb(RF_NODE_T *node, uint8_t indx)
{
	return node->door_status[indx];
}
/************************************************/
void rf_msg_recv_callback(char *msg, COMMAND_RESPONSE_T *commandResponse,  const UART_PORT_T *src_port)
{
	...
	if(le50_is_mounted()){
		PRINT_K("\n**************************\n");
		PRINT_K("Message from RF Node : ");
		PRINT_K(msg);
		PRINT_K("\n**************************\n");
		rf_update_module_alive_timer();
		if(rf_parse_packet(msg, &node))
			...
			//	rf_update_tablet_app(&node);
				break;

				case NODE_TYPE_CAP_SENSOR:
				/* Send alarm information to T2 only if
				 * igniton is not active and vehicle is
				 * stationary
				 * */
				if((CAP_SENSOR_ALARM_ID == node.alarm_status)/* && (!Get_IgnitionStatus()) && (!gps_is_moving())*/){
					/* generate alarm for T message*/
					T_MSG_EVENT_T event;
					memset(&event, 0, sizeof(event));
					strcpy(event.msg_type, EVENT_FUEL_CAP_REMOVED);
					strcpy(event.param1,node.node_id);
					put_t_msg_event(&event);

					/* write event to tablet app */
				/*	TABLET_APP_EVENT_T tablet_event;
					tablet_event.event_type = MOBILE_APP_EVENT_XMIT_FUEL_CAP_ALARM;
					strcpy(tablet_event.json_data_t.json_type_sensor.id, node.node_id);
					put_tablet_app_event(&tablet_event);*/
				}
				break;
...
/***********************************************/
bool rf_parse_packet(char *recv_buffer, RF_NODE_T *node_received)
{
	RF_NODE_T node;
	...
	memcpy(node_received, &node, sizeof(RF_NODE_T));
	/*if(trailer_detection_timer_expired){
