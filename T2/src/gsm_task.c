...
void wrt_gsm_task_event_callback(uint32_t param);
...
extern TIMER_ASYNC *lbsQueryTimerId;
					...
					switch(event.sig){
						case SIGNAL_GSM_TASK_REGISTERED:
							if(settings_check_lbs_activation_status()){
								Timer_Stop(lbsQueryTimerId);
								Timer_Start(lbsQueryTimerId , TIMER_TYPE_CONTINUOUS, LBS_QUERY_TIMEOUT, wrt_gsm_task_event_callback, SIGNAL_GSM_QUERY_LBS_TICK);
							}
			...
			case GSM_TASK_STATE_REGISTERED:
				if(GetEvent_GsmTask(&event)){
					...
					switch(event.sig){
						...
						case SIGNAL_GSM_LBS_ACTIVATED:
							Timer_Stop( lbsQueryTimerId );
							Timer_Start( lbsQueryTimerId, TIMER_TYPE_CONTINUOUS, LBS_QUERY_TIMEOUT, wrt_gsm_task_event_callback, SIGNAL_GSM_QUERY_LBS_TICK);
							break;

						case SIGNAL_GSM_LBS_DEACTIVATED:
							Timer_Stop(lbsQueryTimerId);
							break;

						case SIGNAL_GSM_QUERY_LBS_TICK:
							PRINT_K("\nQuerying LBS...\n");
