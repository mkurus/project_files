...
typedef enum{
	DIN_FUNC_NOT_ASSIGNED,     /* standard digital input function. Send T message on state change */
...
typedef enum {
	...
	DIN_SETTING = 402,
	ENTER_SLEEP_MODE = 403,
	ACTIVATE_ENGINE_HOUR_COUNTER = 404,
	SET_CMD_DOUT_PULSE = 405,
	FIRMWARE_UPDATE_REQUEST = 503,
	SET_OUTPUT_STATUS = 599,
...
typedef struct
{
	...
	uint32_t lbsActStatus;
	uint32_t engineHrCntActStatus;
	uint32_t chksum;
}FLASH_SETTINGS_T;
