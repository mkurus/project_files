...
/*	get_scale_info(&scale_info);
	if(scale_info.isConnected){
		Trio_AddTMessageExtensionSeperator(msgBuffer);
		Trio_PrepareTMessageExtension(msgBuffer, SCALE_INFO);
	}*/
...
*/
	...
	if(settings_check_engine_hour_activation_status()){
		Trio_AddTMessageExtensionSeperator(msgBuffer);
		Trio_PrepareTMessageExtension(msgBuffer, ENGINE_HOUR_VALUE);
	}
...
//	Get_EventInfo(&event_info);
	...
	/*	case SCALE_INFO:
		get_scale_info(&scale_info);
		sprintf(temp,"weight:%d", (int)(scale_info.scaleVal));
...
*/
		...
		case ENGINE_HOUR_VALUE:
		sprintf(temp, "%X", (int)getEngineHourValue());
		strcat(u8_tMessage, "12:");
		strcat(u8_tMessage, temp);
		break;
