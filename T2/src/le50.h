...
#define     LE50_POWER_ON_DELAY         (1*SECOND)
#define     LE50_EVENT_QUE_SIZE         10
#define     LE50_ALIVE_CHECK_TIMEOUT    (1 * MINUTE)
#define     LE50_DETECT_TIMEOUT         (5 * SECOND)

typedef struct LE50_INIT{
	char *command;
	bool (*before_callback)();
	bool (*after_callback)(char *);
	TIMER_TICK_T timeout;
	uint8_t retry;
}LE50_INIT_T;

typedef enum LE50_STATE{
	LE50_SEND_COMMAND_STATE,
	LE50_WAIT_RESPONSE_STATE,
	LE50_IDLE,
}LE50_STATE_T;

typedef enum LE50_EVENT_TYPE{
    LE50_EVENT_INIT = 1,
}LE50_EVENT_TYPE_T;

/*typedef struct LE50_EVENT{
	LE50_EVENT_TYPE event;
}LE50_EVENT_T;*/
