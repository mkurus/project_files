...
/*****************************************************************/
#define DEBUG_UART_SRB_SIZE         (8192) 	                 /* Send ring buffer size */
...
//	{ TABLET_MSG_START_CHAR, TABLET_MSG_END_CHAR,      tablet_msg_recv_callback },     /*tablet application */
	{  LORA_MSG_START_CHAR,    LORA_MSG_END_CHAR,      lora_msg_recv_callback   }      /* LORA message */
};
...
{
	Chip_UART_SendRB(TRACE_PORT, &debug_txring, msg, length);
...
void task_trace()
{
	...
	line_info.u16_outBufferSize = sizeof(debugRxBuffer);
...
char trace_get_char()
{
	...
	while(1){
		if(Chip_UART_ReadRB(TRACE_PORT, &debug_rxring, &ch_input, 1)){
			return ch_input;
		}
	}
}
