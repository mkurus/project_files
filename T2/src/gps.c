		...
		switch(nmea_msg_t->type)
		{
			...
			if( (nmea_msg_t->nmea_msg_items_t.rmc.status == NMEA_RMC_VALID)   &&
			    ...
			    (nmea_msg_t->nmea_msg_items_t.gga.satellites > 5) ){
					...
					PRINT_K("\nSpeed =      ");
					for(i =0; i < G_VAL_WINDOW_SIZE;  i++){
						sprintf(printBuf," %.3f",  gps_acc_info[i].speed);
						PRINT_K(printBuf);
					}

					PRINT_K("\nFiltered G = ");
					for(i =0; i < G_VAL_WINDOW_SIZE;  i++){
					   sprintf(printBuf," %.3f", gps_acc_info[i].filtered_g);
					   PRINT_K(printBuf);
					}

					PRINT_K("\nRaw G =      ");
					for(i =0; i < G_VAL_WINDOW_SIZE;  i++){
						sprintf(printBuf," %.3f",  gps_acc_info[i].g);
					    PRINT_K(printBuf);
					}
					...
					if(mma865x_read_accel_values(&gSensorVal)){
						...
						sprintf(printBuf,"\nXYZ = %.3f  %.3f  %.3f | G=%.3f\r\n", gSensorVal.xAccel,gSensorVal.yAccel, gSensorVal.zAccel,
																													    totalGSensorVal);
						PRINT_K(printBuf);
					}
					if(sample_count < G_VAL_WINDOW_SIZE- 1 )
